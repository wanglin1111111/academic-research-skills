...
# 获取OpenClaw skills目录
SKILLS_DIR="$HOME/.qclaw/skills"
mkdir -p "$SKILLS_DIR"

echo "[1/3] 正在安装 ai-native-creation-mindset..."
if cp -r ai-native-creation-mindset "$SKILLS_DIR/"; then
...
if cp -r ai-creation-prompt-toolkit "$SKILLS_DIR/"; then
...
echo
echo "[3/3] 正在验证安装..."
if [ -f "$SKILLS_DIR/ai-native-creation-mindset/SKILL.md" ] && [ -f "$SKILLS_DIR/ai-creation-prompt-toolkit/SKILL.md" ]; then
